...
uint256_t::uint256_t() : UPPER(uint128_0), LOWER(uint128_0) {}
...
uint256_t::operator bool() const { return (bool)(UPPER | LOWER); }

uint256_t::operator uint8_t() const { return (uint8_t)LOWER; }

uint256_t::operator uint16_t() const { return (uint16_t)LOWER; }

uint256_t::operator uint32_t() const { return (uint32_t)LOWER; }

uint256_t::operator uint64_t() const { return (uint64_t)LOWER; }

uint256_t::operator uint128_t() const { return LOWER; }
...
uint256_t uint256_t::operator~() const { return uint256_t(~UPPER, ~LOWER); }
...
uint256_t uint256_t::operator<<(const uint256_t &rhs) const {
    ...
    if (((bool)rhs.UPPER) || (shift >= uint128_256)) {
        ...
        return uint256_t((UPPER << shift) + (LOWER >> (uint128_128 - shift)),
                         LOWER << shift);
...
uint256_t uint256_t::operator>>(const uint256_t &rhs) const {
    ...
    if (((bool)rhs.UPPER) | (shift >= uint128_256)) {
        ...
        return uint256_t(UPPER >> shift,
                         (UPPER << (uint128_128 - shift)) + (LOWER >> shift));
    } else if ((uint128_256 > shift) && (shift > uint128_128)) {
...
bool uint256_t::operator!() const { return !(bool)*this; }
...
uint256_t uint256_t::operator+(const uint256_t &rhs) const {
    return uint256_t(
        UPPER + rhs.UPPER +
            (((LOWER + rhs.LOWER) < LOWER) ? uint128_1 : uint128_0),
        LOWER + rhs.LOWER);
}
...
uint256_t uint256_t::operator-(const uint256_t &rhs) const {
    return uint256_t(UPPER - rhs.UPPER - ((LOWER - rhs.LOWER) > LOWER),
                     LOWER - rhs.LOWER);
}
...
uint256_t uint256_t::operator*(const uint256_t &rhs) const {
    ...
    uint128_t top[4] = {UPPER.upper(), UPPER.lower(), LOWER.upper(),
                        LOWER.lower()};
    uint128_t bottom[4] = {rhs.upper().upper(), rhs.upper().lower(),
                           rhs.lower().upper(), rhs.lower().lower()};
    ...
    uint128_t third64 =
        uint128_t(products[0][2].lower()) + uint128_t(products[0][3].upper());
    uint128_t second64 =
        uint128_t(products[0][1].lower()) + uint128_t(products[0][2].upper());
    uint128_t first64 =
        uint128_t(products[0][0].lower()) + uint128_t(products[0][1].upper());
...
uint256_t uint256_t::operator+() const { return *this; }

uint256_t uint256_t::operator-() const { return ~*this + uint256_1; }
...
std::string uint256_t::str(uint8_t base, const unsigned int &len) const {
    ...
    if (!(*this)) {
        ...
        do {
            ...
            out = "0123456789abcdefghijklmnopqrstuvwxyz"[(uint8_t)qr.second] +
                  out;
        } while (qr.first);
    }
    if (out.size() < len) {
